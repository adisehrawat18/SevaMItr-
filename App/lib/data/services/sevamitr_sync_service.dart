...
class SevaMitrSyncService extends ChangeNotifier {
  ...
  Future<void> init() async {
    try {
      ...
      await CaregiverAuthService.instance.init();
  ...
  Future<SyncResult> syncAll({bool isBackground = false}) async {
    ...
    try {
      ...
      if (profile != null) {
        ...
        final phoneDigits = (auth.isLoggedIn && auth.caregiverPhone.isNotEmpty
                ? auth.caregiverPhone
                : profile.caregiverPhone)
            .replaceAll(RegExp(r'[^0-9]'), '');
        final isDemo = (auth.isLoggedIn && auth.caregiverPhone.contains('anuradha')) ||
            phoneDigits.contains('9435012345') ||
            phoneDigits.contains('9435122334') ||
            phoneDigits.isEmpty;
        final caregiverId = auth.isLoggedIn && auth.caregiverId.isNotEmpty
            ? auth.caregiverId
            : (isDemo ? 'demo-caregiver-001' : 'cg-ner-$phoneDigits');
        final caregiverName = auth.isLoggedIn && auth.caregiverName.isNotEmpty
            ? auth.caregiverName
            : profile.caregiverName;
        final caregiverPhone = auth.isLoggedIn && auth.caregiverPhone.isNotEmpty
            ? auth.caregiverPhone
            : profile.caregiverPhone;

        String activeServerId = profile.serverPatientId;
        if (activeServerId.isEmpty && caregiverId.isNotEmpty) {
          try {
            final checkUrl = Uri.parse('$_serverBaseUrl/api/patients?caregiverId=$caregiverId');
            final checkRes = await _client.get(checkUrl, headers: {'Accept': 'application/json'}).timeout(const Duration(seconds: 4));
            if (checkRes.statusCode == 200) {
              final checkData = jsonDecode(checkRes.body);
              final pList = checkData['patients'] as List<dynamic>?;
              if (pList != null && pList.isNotEmpty) {
                final match = pList.firstWhere(
                  (p) => (p['fullName'] as String? ?? '').trim().toLowerCase() == profile.fullName.trim().toLowerCase(),
                  orElse: () => pList.first,
                );
                if (match != null && match['id'] != null) {
                  activeServerId = match['id'] as String;
        ...
        final regPayload = {
          if (activeServerId.isNotEmpty) 'id': activeServerId,
        ...
        // If local profile has not been configured yet, ensure demo patient exists on cloud
        final auth = CaregiverAuthService.instance;
        final caregiverId = auth.isLoggedIn && auth.caregiverId.isNotEmpty
            ? auth.caregiverId
            : 'demo-caregiver-001';
        ...
        try {
          final regRes = await _client
              .post(
                Uri.parse('$_serverBaseUrl/api/patients'),
                headers: {'Content-Type': 'application/json'},
                body: jsonEncode({
                  'id': 'patient-ner-001',
          ...
          if (regRes.statusCode == 200 || regRes.statusCode == 201) {
            final regData = jsonDecode(regRes.body);
            if (regData['patient'] != null && regData['patient']['id'] != null) {
              targetPatientId = regData['patient']['id'] as String;
            }
          }
        } catch (e) {
          debugPrint('Default patient registration warning: $e');
        }
      }
      ...
      try {
        ...
        final analyticsUrl = Uri.parse('$_serverBaseUrl/api/analytics?patientId=$targetPatientId&_t=$nowMs');
        final analyticsRes = await _client
            .get(analyticsUrl, headers: {
      ...
      if (analytics == null || analytics.sessionsCount == 0) {
        try {
          final nowMs = DateTime.now().millisecondsSinceEpoch;
          final sessionsUrl = Uri.parse('$_serverBaseUrl/api/patients/$targetPatientId/sessions?_t=$nowMs');
          final sessionsRes = await _client
              .get(sessionsUrl, headers: {
                'Accept': 'application/json',
                'Cache-Control': 'no-cache',
              })
              .timeout(const Duration(seconds: 8));
  ...
  static String _mapGameTypeToSevaMitrGameTitle(String gameType) {
    switch (gameType.toUpperCase()) {
      case 'BIJULI_TAP':
        return 'Bijuli Tap (Reaction Speed & Inhibition)';
      case 'BIKHAMA_KHOJ':
        return 'Bikhama Khoj (Visual Search & Odd One Out)';
      case 'DOUBLE_DECISION':
        return 'Sight Speed (Double Decision UFOV)';
      case 'SOUND_SWEEPS':
        return 'Sound Sweeps (Auditory Temporal Processing)';
      case 'SPEED_MAZE':
        return 'Speed Maze (Spatial Planning & Navigation)';
      case 'TARGET_TRACKER':
        return 'Target Tracker (Multiple Object Attention)';
      case 'MEMORY_MATCH':
  ...
  static ServerAnalytics _deriveAnalyticsFromSessions(String patientId, List<dynamic> sessions) {
    ...
    for (final s in sessions) {
      ...
      } else if (gameId.contains('doharani') || gameId.contains('routine') || gameId.contains('maze')) {
        domainLists['executive']!.add(score);
      } else if (gameId.contains('rang') || gameId.contains('attention') || gameId.contains('tracker') || gameId.contains('decision') || gameId.contains('bikhama')) {
        domainLists['attention']!.add(score);
      } else if (gameId.contains('shabda') || gameId.contains('sound') || gameId.contains('bijuli')) {
