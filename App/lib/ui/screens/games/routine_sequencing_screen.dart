...
class _RoutineSequencingScreenState extends State<RoutineSequencingScreen> {
  ...
  final List<RoutineStep> _allSteps = const [
    ...
    RoutineStep(
      correctOrderIndex: 1,
      title: "routine_bath",
      regionalTitle: "routine_bath",
      icon: Icons.bathtub,
      color: DementiaColors.voiceAssistanceBlue,
    ),
    RoutineStep(
      correctOrderIndex: 2,
      title: "routine_prayer",
      regionalTitle: "routine_prayer",
      icon: Icons.self_improvement,
      color: DementiaColors.alertTerracotta,
    ),
    RoutineStep(
      correctOrderIndex: 3,
      title: "routine_breakfast",
      regionalTitle: "routine_breakfast",
  ...
  Future<void> _recordGameCompletion() async {
    ...
    final score = (100.0 - (_errorCount * 8) - (elapsed > 40 ? 8 : 0))
        .clamp(40.0, 100.0);
    try {
    ...
    showDialog(
      ...
      builder: (_) => GameCelebrationDialog(
        score: score.toInt(),
        biomarker: 'Executive Sequence: ${_allSteps.length - _errorCount}/${_allSteps.length} Correct • ${elapsed}s',
        onPlayAgain: () {
          ...
          children: [
            ...
            if (!_isCompleted) ...[
              ...
              ..._availableOptions.map((option) {
                final optionTitle = context.tr(option.title);
                final regionalTitle = context.tr(option.regionalTitle);
                return Padding(
                  padding: const EdgeInsets.only(bottom: 12),
                  child: AccessibleTouchWrapper(
                    semanticLabel: "$optionTitle $regionalTitle",
